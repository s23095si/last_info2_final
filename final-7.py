...
class Player:
    ...
    def update(self):
        ...
        # マウスの左クリックでプレイヤーの位置を更新
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            self.x, self.y = pyxel.mouse_x - 8, pyxel.mouse_y - 16
...
class Point:
    ...
    def update(self):
        ...
        if self.x < -40:
            self.x += 240
            self.y = randint(0, 104)
            # 新しい画像に切り替わるたびにランダムな画像に変更
            self.image_index = randint(1,2)  # 0, 1, 2のいずれかの画像に変更
